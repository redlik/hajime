                                        ...
                                        @if ($member->getFirstMedia('photo'))
                                            <div class="mb-2">
                                                <img src="{{ $member->getFirstMediaUrl('photo', 'profile') }}"
                                                     class="w-24 h-24 object-contain rounded border border-gray-300" alt="Current photo">
                                                <div class="text-xs text-gray-500 mt-1">
                                                    Uploaded {{ $member->getFirstMedia('photo')->created_at->format('d/m/Y') }}
                                                </div>
                                            </div>
                                        @else
                                            <div class="text-sm text-gray-400 mb-2">No photo on file.</div>
                                        @endif
                                        <input class="inline-block" id="photo" name="photo" type="file"
                                               accept="image/jpeg,image/png,image/webp,image/heic,image/heif">
                                        <p class="text-xs text-gray-500 mt-1">JPG, PNG, WEBP or HEIC, up to 5MB. Uploading a new photo replaces the current one.</p>
                                    </div>
                                </div>
                                <div class="w-full md:w-1/2">
                                    <div class="mb-4">
